...
class AdvancedScenarioConfig:
    ...
    def validate(self) -> None:
        ...
        if not np.isclose(self.outpatient_share + self.inpatient_share + self.emergency_share, 1.0, atol=1e-6):
        ...
        profiles = (*self.weekday_multipliers, *self.seasonal_multipliers, *self.outpatient_hourly_profile, *self.emergency_hourly_profile_24h)
        if any(value < 0 for value in profiles) or sum(self.outpatient_hourly_profile) <= 0 or sum(self.emergency_hourly_profile_24h) <= 0:
...
class AdvancedMRIModel:
    ...
    def patient(self, patient_id: int):
        ...
        try:
            ...
            stages = (
                ...
                ("preparation", self.radiographers, float(self.rng.exponential(self.config.preparation_mean))),
            )
            for stage, capacity, duration in stages:
                wait = yield self.env.process(self._timed_staff_service(capacity, duration, deadline))
                if wait is None:
            ...
            self._finish(patient_id, "completed", **waits, wait_minutes=total_wait,
    ...
    def _effective_daily_slots(self, requested: int) -> int:
        ...
        maintenance = sum(min(window.end, open_minutes) - window.start for window in self.config.machine_maintenance if window.start < open_minutes)
        ...
        failure_fraction = 0.0 if not mtbf else min(0.95, float(np.mean([
            self.config.machine_repair_mean_minutes / (value + self.config.machine_repair_mean_minutes) for value in mtbf
        ])))
        usable = planned_capacity * (1 - failure_fraction) * (1 - self.config.emergency_capacity_reserve)
    ...
    def source(self):
        ...
        for day in range(total_days):
            ...
            slot_count = self._effective_daily_slots(int(round(demand * self.config.outpatient_share)))
            if slot_count:
                ...
                for hour, count in enumerate(per_hour):
                    ...
                    for index in range(count):
                        ...
                        self.ledger[patient_id] = {
                            "patient_id": patient_id, "patient_type": "outpatient", "booked_time": day_start,
                            "scheduled_time": scheduled, "status": "booked", "entered_system": False,
                        }
                        if self.rng.random() < self.config.cancellation_rate:
                            self.ledger[patient_id].update(
                                status="cancelled", outcome_time=max(day_start, scheduled - self.config.cancellation_lead_minutes)
                            )
                            continue
                        jitter = float(self.rng.normal(0, self.config.appointment_arrival_sd_minutes))
                        events.append((min(day_start + open_minutes - 0.01, max(day_start, scheduled + jitter)), patient_id))

            for offset in self.rng.uniform(0, open_minutes, int(self.rng.poisson(demand * self.config.inpatient_share))):
                patient_id += 1
                arrival = day_start + float(offset)
                self.ledger[patient_id] = {
                    "patient_id": patient_id, "patient_type": "inpatient", "booked_time": arrival,
                    "scheduled_time": np.nan, "status": "expected", "entered_system": False,
                }
                events.append((arrival, patient_id))

            for hour, weight in enumerate(emergency_profile):
                for offset in self.rng.uniform(0, 60, int(self.rng.poisson(demand * self.config.emergency_share * float(weight)))):
                    patient_id += 1
                    arrival = day_start + hour * 60 + float(offset)
                    self.ledger[patient_id] = {
                        "patient_id": patient_id, "patient_type": "emergency", "booked_time": arrival,
                        "scheduled_time": np.nan, "status": "expected", "entered_system": False,
                    }
                    events.append((arrival, patient_id))
...
def _run_to_termination(env: simpy.Environment, model: AdvancedMRIModel, config: AdvancedScenarioConfig, horizon: float) -> None:
    env.run(until=horizon)
    if config.termination_policy == "horizon":
        return
    drain_limit = float("inf") if config.termination_policy == "drain" else horizon + config.max_drain_minutes
...
def run_advanced_once(config: AdvancedScenarioConfig, replication: int = 0) -> tuple[AdvancedSimulationResult, pd.DataFrame, pd.DataFrame]:
    ...
    if frame.empty:
        frame = pd.DataFrame(columns=["patient_id", "patient_type", "status", "booked_time", "entered_system"])
    frame = frame.loc[(frame["booked_time"].astype(float) >= measurement_start) & (frame["booked_time"].astype(float) < horizon)].copy()
    ...
    result = AdvancedSimulationResult(
        scenario=config.name, replication=replication, booked=booked, cancelled=cancelled,
        expected_arrivals=int(expected_arrivals), no_shows=no_shows, arrivals=arrivals, completed=completed,
        abandoned=abandoned, unfinished=unfinished,
        ...
        mean_queue_length=mean_queue_open, mean_queue_length_open=mean_queue_open,
        ...
        mean_available_mri=mean_available_open, mean_available_mri_open=mean_available_open,
