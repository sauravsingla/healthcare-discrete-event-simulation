...
class ScenarioConfig:
    ...
    def validate(self) -> None:
        ...
        if self.hourly_arrival_profile and (
            any(value < 0 for value in self.hourly_arrival_profile)
            or sum(self.hourly_arrival_profile) <= 0
        ):
        ...
        if any(
            value < 0
            for value in (self.scan_sd_outpatient, self.scan_sd_inpatient, self.scan_sd_emergency)
        ):
...
class MRIModel:
    ...
    def _next_available_delay(
        self, windows: tuple[tuple[int, int], ...], include_breaks: bool = True
    ) -> float:
        ...
        in_break = include_breaks and any(
            start <= minute < end for start, end in self.config.staff_breaks
        )
        ...
        if in_break:
            candidates.extend(
                end - minute for start, end in self.config.staff_breaks if start <= minute < end
            )
            ...
            self.records.append(
                {
                    ...
                    "arrival_deviation_minutes": (arrival - scheduled_time)
    ...
    def _launch(
        self, patient_id: int, patient_type: str, scheduled_time: float | None = None
    ) -> None:
    ...
    def source(self):
        ...
        raw_profile = np.asarray(
            self.config.hourly_arrival_profile or DEFAULT_HOURLY_PROFILE, dtype=float
        )
        profile = np.interp(
            np.linspace(0, 1, self.config.operating_hours),
            np.linspace(0, 1, len(raw_profile)),
            raw_profile,
        )
        profile = profile / profile.sum()
        outpatient_daily = self.config.daily_demand * self.config.outpatient_share
        unscheduled_daily = self.config.daily_demand * (
            self.config.inpatient_share + self.config.emergency_share
        )
        ...
        for day in range(total_days):
            ...
            slots = np.arange(
                0, open_minutes, self.config.appointment_interval_minutes, dtype=float
            )
            ...
            for slot in slots[:slot_count]:
                ...
                arrival_time = max(
                    day_start, min(day_start + open_minutes - 0.01, day_start + slot + jitter)
                )
            ...
            for hour, weight in enumerate(profile):
                ...
                for offset in self.rng.uniform(0, 60, count):
                    patient_type = (
                        "inpatient" if self.rng.random() < inpatient_probability else "emergency"
                    )
...
def run_once(config: ScenarioConfig, replication: int = 0) -> tuple[SimulationResult, pd.DataFrame]:
    ...
    result = SimulationResult(
        ...
        mri_failures=model.mri_failures,
        mri_downtime_minutes=model.mri_downtime_minutes,
    )
    return result, frame
