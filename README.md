...
> **Dataset scope:** these are three synthetic workload datasets generated by the documented simulation configuration, not three independent external clinical datasets. The repository includes a public NHS aggregate-data preparation and provenance workflow, but separate multi-source clinical benchmark results should not be claimed until authoritative external datasets are calibrated, executed and independently validated.
